...
export function Projects({ projects }) {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.items.filter((project) => project.category === activeFilter);
      ...
      >
        <div className="grid lg:grid-cols-[1.08fr_0.92fr]">
          <div className="relative min-h-[28rem] overflow-hidden">
            <img src={featured.image} alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover transition duration-700 hover:scale-105" />
          ...
          <div className="p-6 sm:p-8 lg:p-10">
            <p className="text-sm font-semibold text-mint">{featured.category} / {featured.year}</p>
            <h3 className="mt-4 font-display text-3xl font-semibold text-cloud sm:text-4xl">{featured.title}</h3>
            <p className="mt-4 text-base leading-7 text-mist">{featured.summary}</p>
            <p className="mt-5 rounded-2xl border border-line bg-black/20 p-4 text-sm leading-6 text-cloud">{featured.impact}</p>
            <div className="mt-6 flex flex-wrap gap-2">
              {featured.stack.map((tag) => (
                <span key={tag} className="rounded-full border border-line bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-mist">
                  {tag}
                </span>
              ))}
            </div>
            <div className="mt-7 flex flex-wrap gap-3">
              {featured.links.map((link) => (
                <MagneticButton key={link.label} href={link.href} variant="secondary" icon={link.icon} className="min-h-11">
                  {link.label}
                </MagneticButton>
              ))}
            </div>
  ...
  return (
    <GlassCard as={motion.article} layout className="group p-0">
      <div className="relative aspect-[1.35] overflow-hidden">
        <img src={project.image} alt="" loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
      ...
      <div className="p-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h3 className="font-display text-xl font-semibold text-cloud">{project.title}</h3>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {project.links.map((link) => (
            <a key={link.label} href={link.href} className="inline-flex items-center gap-2 rounded-full border border-line px-3 py-2 text-xs font-semibold text-cloud transition hover:border-mint/40">
